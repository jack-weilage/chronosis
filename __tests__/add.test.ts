...
test('second', () => {
	...
	// Within normal range
	const normal = chrono.clone().add(15, 'second')
	expect(normal.get('second')).toBe(44)
	expect(normal.get('minute')).toBe(29)

	// Should add one to the next unit
	const wrap_forward = chrono.clone().add(45, 'second')
...
test('minute', () => {
	...
	// Within normal range
	const normal = chrono.clone().add(15, 'minute')
	expect(normal.get('minute')).toBe(44)
	expect(normal.get('hour')).toBe(11)

	// Should add one to the next unit
	const wrap_forward = chrono.clone().add(35, 'minute')
...
test('hour', () => {
	...
	// Within normal range
	const normal = chrono.clone().add(8, 'hour')
	expect(normal.get('hour')).toBe(19)
	expect(normal.get('day')).toBe(15)

	// Should add one to the next unit
	const wrap_forward = chrono.clone().add(25, 'hour')
	expect(wrap_forward.get('hour')).toBe(12)
...
test('day', () => {
	...
	// Within normal range
	const normal = chrono.clone().add(10, 'day')
	expect(normal.get('day')).toBe(25)
	expect(normal.get('month')).toBe(5)

	// Should add one to the next unit
	const wrap_forward = chrono.clone().add(20, 'day')
...
test('month', () => {
	...
	// Within normal range
	const normal = chrono.clone().add(3, 'month')
	expect(normal.get('month')).toBe(8)
	expect(normal.get('year')).toBe(2020)

	// Should add one to the next unit
	const wrap_forward = chrono.clone().add(18, 'month')
...
test('year', () => {
	...
	// Within normal range
	const normal = chrono.clone().add(3, 'year')
